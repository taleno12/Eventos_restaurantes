...
<head>
    ...
    <title>Ordenar — {{ $gastrobar->nombre }} | Gastro Nicaragua</title>
...
<style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .topbar { background: var(--white); border-bottom: 1px solid var(--border); position: sticky; top: 0; z-index: 9999; height: 56px; display: flex; align-items: center; padding: 0 24px; gap: 16px; box-shadow: 0 4px 0 var(--white); will-change: transform; }
    ...
    .rest-hero { border-bottom: 1px solid var(--border); padding: 20px 24px 0; background: var(--white); position: relative; z-index: 1; }
    ...
    .search-wrap { max-width: 1200px; margin: 0 auto; padding: 0 24px 16px; background: var(--white); position: relative; z-index: 1; }
    ...
    .cats-bar { position: sticky; top: 56px; z-index: 9998; background: var(--white); border-bottom: 1px solid var(--border); box-shadow: 0 4px 0 var(--white); will-change: transform; }
    ...
    .page-body { flex: 1; max-width: 1200px; margin: 0 auto; width: 100%; padding: 28px 24px 80px; display: grid; grid-template-columns: 1fr 360px; gap: 28px; align-items: start; align-content: start; position: relative; z-index: 0; }
    ...
    .modal-overlay { display: none; position: fixed; inset: 0; z-index: 99999; background: rgba(0,0,0,0.55); align-items: flex-end; justify-content: center; }
...
{{-- TOPBAR --}}
<header class="topbar">
    <a href="{{ route('gastrobares.show', $gastrobar) }}" class="btn-back">
        <i class="fas fa-arrow-left" style="font-size:12px;"></i>
    </a>
    <div class="topbar-brand">
        <i class="fas fa-cocktail" style="color:var(--blue);font-size:12px;"></i>
        Ordenar
        <span class="sep">/</span>
        <span class="name">{{ $gastrobar->nombre }}</span>
    </div>
    @auth
...
<div class="rest-hero">
    <div class="rest-hero-inner">
        <div class="rest-logo">
            @php
                $imgUrl = $gastrobar->imagen_principal
                    ? asset('storage/'.$gastrobar->imagen_principal)
                    : ($gastrobar->galeria && count($gastrobar->galeria) > 0
                        ? asset('storage/'.$gastrobar->galeria[0])
                        : null);
            @endphp
            @if($imgUrl)
                <img src="{{ $imgUrl }}" alt="{{ $gastrobar->nombre }}">
            @else
                <i class="fas fa-cocktail" style="font-size:24px;color:#d1d5db;"></i>
            @endif
        </div>
        <div class="rest-info">
            ...
            <div class="rest-meta">
                ...
                <span>{{ optional($gastrobar->municipio)->nombre }}, {{ $gastrobar->departamento->nombre }}</span>
                @if($gastrobar->tipo_bar)
                    <span class="rest-meta-sep">·</span>
                    <span>{{ $gastrobar->tipo_bar }}</span>
                @endif
        ...
        <div class="carrito-card" id="carrito-card">
            ...
            <div class="carrito-body">
                ...
                <div id="carrito-summary" class="carrito-summary" style="display:none;">
                    ...
                    <form method="POST" action="{{ route('pedidos.gastrobar.store', $gastrobar) }}" id="form-pedido">
                        @csrf
...
function abrirModal(id, nombre, precio, desc, imgUrl, opciones) {
    ...
    wrap.innerHTML = imgUrl
        ...
        : `<div class="modal-img-placeholder"><i class="fas fa-cocktail"></i></div>`;
