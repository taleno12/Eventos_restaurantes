...
class EnviarNotificacionesVencimientoGastrobar extends Command
{
    ...
    public function handle(): void
    {
        ...
        $contratos = Contrato::whereNotNull('gastrobar_id')
            ->whereNotNull('fecha_fin')
            ->where('estado', 'activo')
            ->with('gastrobar.propietario')
            ->get();

        foreach ($contratos as $contrato) {
            $gastrobar = $contrato->gastrobar;
            if (!$gastrobar) continue;

            $user = $gastrobar->propietario;
            if (!$user) continue;

            $vence     = \Carbon\Carbon::parse($contrato->fecha_fin)->startOfDay();
            ...
            $configGastrobar = match (true) {
                $diasResta === 7 => [
                    ...
                    'mensaje' => "Tu membresía del plan {$contrato->plan} vence el {$vence->format('d/m/Y')}. Renuévala para seguir disfrutando de todos los beneficios de GastroNicaragua.",
                ...
                $diasResta === 3 => [
                    ...
                    'mensaje' => "Solo te quedan 3 días. Tu membresía del plan {$contrato->plan} vence el {$vence->format('d/m/Y')}. Contáctanos para renovar y evitar la suspensión de tu cuenta.",
                ...
                $diasResta === 0 => [
                    ...
                    'mensaje' => "Tu membresía del plan {$contrato->plan} vence hoy. Contáctanos de inmediato al +505 8540 6068 para renovar y evitar que tu gastrobar sea suspendido.",
            ...
            $configAdmin = match (true) {
                $diasResta === 7 => [
                    ...
                    'mensaje' => "El gastrobar {$gastrobar->nombre} (plan {$contrato->plan}) vence el {$vence->format('d/m/Y')}. Considera contactar al propietario para renovar.",
                    'icono'   => 'bi-clock-history',
                    'color'   => 'orange',
                    'url'     => '/contratos',
                ],
                $diasResta === 3 => [
                    ...
                    'mensaje' => "El gastrobar {$gastrobar->nombre} (plan {$contrato->plan}) vence el {$vence->format('d/m/Y')}. Se recomienda contactar al propietario a la brevedad.",
                    'icono'   => 'bi-exclamation-triangle-fill',
                    'color'   => 'red',
                    'url'     => '/contratos',
                ],
                $diasResta === 0 => [
                    ...
                    'mensaje' => "El gastrobar {$gastrobar->nombre} (plan {$contrato->plan}) vence hoy. Contacta al propietario de inmediato para renovar o suspender la cuenta.",
                    'icono'   => 'bi-x-circle-fill',
                    'color'   => 'red',
                    'url'     => '/contratos',
                ],
