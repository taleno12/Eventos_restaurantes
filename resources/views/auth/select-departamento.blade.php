...
<head>
    ...
    <title>¿De dónde eres? — GastroNicaragua</title>
...
<body>
    <div class="wrapper">
        ...
        <div class="card">
            ...
            <div class="card-header">
                ...
                <h1 class="card-title">¿De dónde eres?</h1>
                <p class="card-subtitle">
                    Hola, <span>{{ auth()->user()->name }}</span> — elige tu departamento
                </p>
                <p class="card-desc">
                    Te mostraremos eventos, restaurantes y empleos cerca de ti.<br>
                    Siempre podrás cambiar esto más tarde.
                </p>
            </div>

            <form method="POST" action="{{ route('usuario.departamento.save') }}">
                @csrf
                <input type="hidden" name="departamento_id" id="departamentoInput" value="">

                <div class="depto-grid">
                    @foreach($departamentos as $depto)
                        <div class="depto-card" data-id="{{ $depto->id }}" onclick="selectDepto({{ $depto->id }}, this)">
        ...
        <div class="skip-wrap">
            <a href="{{ route('usuario.departamento.skip') }}" class="skip-link">
                ...
                Saltar — ver todos los departamentos
            </a>
        </div>
    ...
    <script>
    function selectDepto(id, el) {
        document.querySelectorAll('.depto-card').forEach(c => c.classList.remove('selected'));
        el.classList.add('selected');
        document.getElementById('departamentoInput').value = id;
