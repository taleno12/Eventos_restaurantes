    ...
    @else
        ...
        <div class="d-flex gap-2 flex-wrap mb-3">
            <button class="btn btn-sm btn-outline-secondary rounded-pill filtro-btn active" data-estado="todos">
                Todos <span class="badge ms-1" style="background:var(--muted);color:white;">{{ $solicitudes->count() }}</span>
            </button>
                ...
                <div class="panel-card h-100">
                    <div class="card-body p-4">
                        ...
                        <div class="d-flex align-items-start justify-content-between mb-3">
                            <div class="d-flex align-items-center gap-3">
                                <div class="rounded-circle d-flex align-items-center justify-content-center flex-shrink-0"
                                     style="width:46px;height:46px;background:rgba(var(--primary-rgb,99,102,241),0.12);">
                            ...
                            @php
                                $badgeColors = [
                                    ...
                                    'vista'      => ['bg' => 'rgba(100,116,139,0.15)', 'text' => '#94a3b8'],
                            ...
                            @endphp
                            <span class="badge rounded-pill" style="background:{{ $badgeColors[$sol->estado]['bg'] }};color:{{ $badgeColors[$sol->estado]['text'] }};">
                                {{ $labelMap[$sol->estado] }}
                            </span>
                        </div>
                        ...
                        <div class="d-flex flex-column gap-1 mb-3">
                            @if(str_ends_with($sol->email, '@telefono.gastronicaragua.local'))
                                <span class="small" style="color:var(--muted);">
                                    <i class="bi bi-telephone-fill me-1" style="color:var(--primary);"></i>Registrado por teléfono
                                </span>
                            @else
                                <a href="mailto:{{ $sol->email }}" class="text-decoration-none small" style="color:var(--muted);">
                                    <i class="bi bi-envelope-fill me-1" style="color:var(--primary);"></i>{{ $sol->email }}
                                </a>
                            @endif
                        ...
                        <div class="d-flex gap-2 justify-content-between align-items-center border-top pt-3" style="border-color:var(--card-border) !important;">
                            <form action="{{ route('gastrobar.solicitudes.estado', $sol) }}" method="POST" class="d-flex gap-1 flex-wrap">
                                @csrf
                                @method('PATCH')
                            ...
                            <form action="{{ route('gastrobar.solicitudes.destroy', $sol) }}" method="POST"
