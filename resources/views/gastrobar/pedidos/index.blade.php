                    ...
                    <tbody class="border-top-0">
                        ...
                        <tr class="fila-pedido border-bottom"
                            ...
                            <td class="py-3 text-center" onclick="event.stopPropagation()">
                                ...
                                <form method="POST" action="{{ route('gastrobar.pedidos.estado', $pedido) }}" class="form-estado">
                                    @csrf @method('PATCH')
                                    <select name="estado"
...
const estadosInfo = @json(\App\Models\PedidoGastrobar::ESTADOS);
</script>
...
// ── CAMBIAR ESTADO ──
// 'cancelado' -> elimina la fila con animación (comportamiento original)
// cualquier otro estado -> se envía vía fetch y se actualiza el badge sin recargar
document.querySelectorAll('.select-estado').forEach(select => {
    select.addEventListener('change', function (e) {
        ...
        if (nuevoEstado === 'cancelado') {
            // ── Lógica existente: elimina la fila con animación ──
            ...
            // ── Resto de estados: enviar al backend y reflejar el cambio en la fila ──
            const formData = new FormData(form);

            // Feedback visual inmediato: deshabilitar el select mientras se procesa
            this.disabled = true;
            ...
            .then(data => {
                ...
                // Si pasó a "entregado", reemplazar el select por el check de completado
                if (nuevoEstado === 'entregado') {
                    const celdaAccion = form.closest('td');
                    if (celdaAccion) {
                        celdaAccion.innerHTML = `<span class="small" style="color:var(--muted) !important;">✓ Completado</span>`;
                    }
                } else {
                    // Quitar la opción "cancelado" del select si ya no aplica (igual que en el render inicial)
                ...
                // Refrescar también los datos del modal de detalle por si lo abren después
